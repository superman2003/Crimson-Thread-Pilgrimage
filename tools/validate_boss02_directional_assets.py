...
CH02_CONFIG = GODOT_ROOT / "data" / "demo_ch02_rain_foundry_canal.json"
EXPECTED_SPRITE = f"res://assets/sprites/bosses/{BOSS_NAME}/manifest.json"
...
MAX_LARGE_EXTRA_COMPONENTS = 3
MAX_EXTRA_COMPONENT_PIXELS = 1550
MIN_VISIBLE_PIXELS = 2400


def main() -> None:
    errors: list[str] = []
    manifest = read_json(BOSS_MANIFEST, errors)
    config = read_json(CH02_CONFIG, errors)
    if not isinstance(manifest, dict) or not isinstance(config, dict):
        raise AssertionError("\n".join(errors) if errors else "invalid input json")
    ...
    validate_ch02_config(config, errors)

    if errors:
        raise AssertionError("BOSS02_DIRECTIONAL_ASSET_VALIDATION_FAIL\n" + "\n".join(f"- {error}" for error in errors))
    ...
    print(
        "BOSS02_DIRECTIONAL_ASSET_VALIDATION_PASS "
        f"animations={checked_animations} frames={checked_frames} "
        f"manifest={normalize_rel(BOSS_MANIFEST)} ch02_sprite={EXPECTED_SPRITE} "
...
def validate_manifest_identity(manifest: dict[str, Any], errors: list[str]) -> None:
    if manifest.get("id") != "boss_02":
        errors.append(f"manifest id must be boss_02, got {manifest.get('id')!r}")
    if "Crimson Thread Scissor Apostle" not in str(manifest.get("name_en", "")):
        errors.append("manifest name_en must be Crimson Thread Scissor Apostle")
    if BOSS_NAME not in str(manifest.get("source_concept", "")):
        errors.append("manifest source_concept must point at the high-fidelity boss_02 concept")
...
def validate_ch02_config(config: dict[str, Any], errors: list[str]) -> None:
    boss = config.get("boss", {})
    if not isinstance(boss, dict):
        errors.append("CH02 config boss must be an object")
        return
    sprite = str(boss.get("sprite", ""))
    if sprite != EXPECTED_SPRITE:
        errors.append(f"CH02 boss sprite must use high-fidelity boss_02 manifest, got {sprite!r}")
    if "third_party" in sprite or "sunken_hammer_smith" in sprite:
        errors.append(f"CH02 boss sprite still points at old placeholder path: {sprite}")
    if str(boss.get("visual_source", "")).lower().find("high-fidelity") < 0:
        errors.append("CH02 boss visual_source must mention high-fidelity source")
    profiles = config.get("ai_profiles", {})
    profile = profiles.get(str(boss.get("kind", "")), {}) if isinstance(profiles, dict) else {}
    attacks = profile.get("attacks", []) if isinstance(profile, dict) else []
    animation_by_id = {
        "hammer_overhead_slam": "boss_02_atk_01",
        "forge_charge": "boss_02_atk_02",
        "anvil_hook_swing": "boss_02_atk_03",
        "triple_ring_quake": "boss_02_atk_03",
    }
    seen: set[str] = set()
    for attack in attacks:
        ...
        if attack_id not in animation_by_id:
            continue
        seen.add(attack_id)
        expected = animation_by_id[attack_id]
        actual = str(attack.get("animation", ""))
        if actual != expected:
            errors.append(f"CH02 attack {attack_id} must use animation {expected}, got {actual!r}")
    missing = sorted(set(animation_by_id) - seen)
    if missing:
        errors.append(f"CH02 boss profile missing mapped attacks: {', '.join(missing)}")
