...
def main() -> None:
    ...
    if errors:
        raise AssertionError("BOSS05_DIRECTIONAL_ASSET_VALIDATION_FAIL\n" + "\n".join(f"- {error}" for error in errors))
    ...
    print(
        "BOSS05_DIRECTIONAL_ASSET_VALIDATION_PASS "
...
def validate_manifest_identity(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    if BOSS_NAME not in str(manifest.get("source_concept", "")):
        errors.append("manifest source_concept must point at the high-fidelity boss_05 concept")
